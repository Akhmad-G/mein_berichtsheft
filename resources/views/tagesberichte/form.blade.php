...
>
    ...
    <x-slot name="icon">
      ...
      >
        <path stroke-linecap="round"
              ...
              d="M6.75 3v2.25M17.25 3v2.25M3.75 8.25h16.5M5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V6.75A2.25 2.25 0 0 0 18.75 4.5H5.25A2.25 2.25 0 0 0 3 6.75v12A2.25 2.25 0 0 0 5.25 21Z"
        />
      </svg>
    </x-slot>
  ...
  <input type="hidden"
         id="wochentag"
         name="wochentag"
         value="{{ old('wochentag', $report['wochentag'] ?? '') }}"
  >
  <input type="hidden"
         id="ausbildungsjahr"
         name="ausbildungsjahr"
         value="{{ old('ausbildungsjahr', $report['ausbildungsjahr'] ?? '') }}"
  >
  <input type="hidden"
         id="ausbildungswoche"
         name="ausbildungswoche"
         value="{{ old('ausbildungswoche', $report['ausbildungswoche'] ?? '') }}"
  >

  <x-report-meta-card title="Berichtsinformationen"
                      :items="[
  ...
  <x-report-section title="Tätigkeiten">
    ...
    <x-textarea-input name="taetigkeiten"
                      id="taetigkeiten"
                      cols="30"
                      rows="10"
                      :readonly="$readonly"
    >{{ old('taetigkeiten', $report['taetigkeiten'] ?? '') }}</x-textarea-input>
    <x-input-error :messages="$errors->get('taetigkeiten')"
                   class="mt-2"
    />
  </x-report-section>
  ...
  <x-report-section title="Was habe ich gelernt?">
    <x-slot name="icon">
      ...
      >
        <path stroke-linecap="round"
              stroke-linejoin="round"
              d="M12 18.75a6.75 6.75 0 0 0 6.75-6.75V6.75A2.25 2.25 0 0 0 16.5 4.5h-9A2.25 2.25 0 0 0 5.25 6.75V12A6.75 6.75 0 0 0 12 18.75Zm0 0v2.25m-3.75 0h7.5"
        />
      </svg>
    </x-slot>

    <x-textarea-input name="gelernt"
                      id="gelernt"
                      cols="30"
                      rows="10"
                      :readonly="$readonly"
  ...
  <x-report-section title="Besondere Ereignisse / Probleme">
    <x-slot name="icon">
      ...
      >
        <path stroke-linecap="round"
              stroke-linejoin="round"
              d="M12 9v3.75m9.303 3.376c.866 1.5-.217 3.374-1.948 3.374H4.645c-1.731 0-2.814-1.874-1.948-3.374l7.355-12.74c.866-1.5 3.03-1.5 3.896 0l7.355 12.74ZM12 15.75h.007v.008H12v-.008Z"
        />
      </svg>
    </x-slot>

    <x-textarea-input name="probleme"
                      id="probleme"
                      cols="30"
                      rows="10"
                      :readonly="$readonly"
