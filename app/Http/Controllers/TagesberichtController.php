...
class TagesberichtController extends Controller {
  ...
  public function create(Request $request) {
    ...
    return view('tagesberichte.create', ['user' => $request->user(), 'ausbildungsbeginn' => auth()->user()->ausbildungsbeginn?->format('Y-m-d'),]);
  }
  ...
  public function store(Request $request, GitLabServiceInterface $gitLabService) {
    ...
    $tagesbericht = $request->validate(['date' => 'required|date', 'wochentag' => 'required|string', 'ausbildungsjahr' => 'required|numeric', 'ausbildungswoche' => ['required', 'string', 'max:255'], 'taetigkeiten' => 'required', 'gelernt' => '', 'probleme' => '',]);
    //      dump($tagesbericht);
  ...
  public function show(string $path, GitLabServiceInterface $gitLabService) {
    ...
    return view('tagesberichte.show', ['report' => $report, 'path' => $realPath, 'canManage' => $canManage,]);
  }
  ...
  public function edit(string $id, GitLabServiceInterface $gitLabService) {
    ...
    return view('tagesberichte.edit', ['report' => $report, 'path' => $id,]);
  }
  ...
  public function update(Request $request, string $id, GitLabServiceInterface $gitLabService) {
    ...
    $tagesbericht = $request->validate(['date' => 'required|date', 'wochentag' => 'required|string', 'ausbildungsjahr' => 'required|numeric', 'ausbildungswoche' => ['required', 'string', 'max:255'], 'taetigkeiten' => 'required', 'gelernt' => 'nullable|string', 'probleme' => 'nullable|string',]);

    $filename = basename($realPath);

    $gitLabService->saveReport($request->user(), $filename, $tagesbericht, 'update');

    return redirect()->route('tagesberichte.show', ['path' => $id])->with('success', 'Tagesbericht aktualisiert.');
  }
  ...
  public function destroy(string $id, GitLabServiceInterface $gitLabService) {
    ...
    return redirect()->route('dashboard')->with('success', 'Tagesbericht gelöscht.');
  }
}
